...
✨ 技术亮点
原子扣库存：基于 Redis 单线程特性 + Lua 脚本，保证库存不超卖

异步下单：RabbitMQ 消峰填谷，避免数据库被瞬时流量打崩

最终一致性：本地消息表 + 定时补偿，保证 Redis 与 MySQL 数据最终一致

分布式锁：基于 Redisson，防止同一用户重复下单

多级缓存：Caffeine（本地）+ Redis（分布式）双重缓存，降低 DB 压力

全局唯一 ID：雪花算法生成订单 ID 和消息 ID，支持分布式部署

安全防护：隐藏秒杀地址 + 验证码 + IP/用户防刷 + 分布式限流
